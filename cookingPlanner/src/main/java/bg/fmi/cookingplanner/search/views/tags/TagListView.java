package bg.fmi.cookingplanner.search.views.tags;
...
public class TagListView extends ViewGroup implements OnHierarchyChangeListener {
    ...
    public static interface TagListener {
        void onAddedTag(TagView view, String tag);

        void onRemovedTag(TagView view, String tag);
    }
    ...
    protected void onMeasure(int widthMeasureSpec, int heightMeasureSpec) {
        ...
        for (int i = 0; i < count; i++) {
            ...
            if (child.getVisibility() != GONE) {
                ...
                lineHeight = Math.max(lineHeight, child.getMeasuredHeight() + 5
                        + lp.mVerticalSpacing);
    ...
    public void onChildViewAdded(View parent, View child) {
        if (child instanceof TagView) {
            ...
            for (TagListener listener : mListeners) {
                listener.onAddedTag(tagView, tagView.getText().toString());
            }
        }
    }
    ...
    public void onChildViewRemoved(View parent, View child) {
        if (child instanceof TagView) {
            ...
            for (TagListener listener : mListeners) {
                listener.onRemovedTag(tagView, tagView.getText().toString());
            }
        }
    }
}